...
function MobileProducts(){
    ...
    return (
        ...
        <div className="w-full py-5 px-40 flex justify-start items-center">
            ...
            <h1 className="text-slate-900 font-medium text-sm"> Mobiles</h1>
        </div>
        <motion.div initial={{y: 35, opacity: 0}} animate={{y: 0, opacity:1}} transition={{duration:0.8}} className="w-full p-5 flex gap-3 flex-wrap justify-center items-center">
            {products.filter((product) => product.category === "Mobiles").map((product) => (
                <div
                    to={`/user/producthome/${product._id}`}
                    key={product._id}
                    className="w-1/5 bg-white h-80 p-5 border border-slate-300 flex justify-center items-center gap-3 rounded-lg shadow-lg flex-col "
                >
                    <img
